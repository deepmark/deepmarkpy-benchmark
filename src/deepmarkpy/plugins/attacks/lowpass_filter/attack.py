...
class LowpassFilterAttack(BaseAttack):
    ...
    def apply(self, audio: np.ndarray, **kwargs) -> np.ndarray:
        ...
        Args:
            ...
            **kwargs: Additional parameters for the lowpass attack:
                ...
                - cutoff_freq_lowpass (float): Optional override of the cutoff frequency in Hz.
        ...
        cutoff_override = kwargs.get("cutoff_freq_lowpass", None)
    ...
    def _lookup_cutoff(self, sampling_rate: int) -> float:
        ...
        """
        per_sr = self.config.get("cutoff_freq_per_sr_lowpass", {})
        if not per_sr:
            raise ValueError(
