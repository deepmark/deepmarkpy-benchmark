...
from deepmarkpy.utils.param_aliases import LEGACY_PARAM_ALIASES, namespaced_dest, new_flag_for_legacy
...
def _add_config_argument(parser, flag, dest, default_value, help_text):
    if isinstance(default_value, bool):
        parser.add_argument(
            flag,
...
def _warn_legacy_flags(argv):
    used = {arg.split("=", 1)[0] for arg in argv}
    legacy_names = {
        legacy_name
        for aliases in LEGACY_PARAM_ALIASES.values()
        for legacy_name in aliases
    }
    for legacy_name in sorted(legacy_names):
        legacy_flag = f"--{legacy_name}"
        if legacy_flag in used:
            replacement = new_flag_for_legacy(legacy_name)
            if replacement:
                logger.warning(
                    "Deprecated flag %s used; prefer %s",
                    legacy_flag,
                    replacement,
                )

def build_parser(models, attacks, valid_args):
    ...
    # Dynamically add namespaced attack configuration parameters.
    for attack_key, params in valid_args.items():
        for arg, default_value in params.items():
            dest = namespaced_dest(attack_key, arg)
            _add_config_argument(
                parser,
                f"--{attack_key}.{arg}",
                dest,
                default_value,
                f"Set {attack_key}.{arg} (default: {default_value})",
            )

    # Legacy flat aliases keep old CLI invocations working for one release.
    for attack_key, aliases in LEGACY_PARAM_ALIASES.items():
        params = valid_args.get(attack_key, {})
        for legacy_name, canonical_name in aliases.items():
            default_value = params.get(canonical_name)
            _add_config_argument(
                parser,
                f"--{legacy_name}",
                namespaced_dest(attack_key, canonical_name),
                default_value,
                (
                    f"Deprecated alias for --{attack_key}.{canonical_name}"
...
def main():
    ...
    parser = build_parser(models, attacks, valid_args)

    args = parser.parse_args()
    _warn_legacy_flags(sys.argv[1:])
