...
class Benchmark:
    ...
    def get_available_args(self):
        ...
        for attack in attacks:
            entry = self.attacks[attack]
            attack_key = entry.get("attack_key", attack)
            config = entry["config"]
            if config is not None:
                valid_args[attack_key] = {
                    key: value
    ...
    def _attack_key_for_class(self, attack_class_name: str) -> str:
        entry = self.attacks.get(attack_class_name) or {}
        return entry.get("attack_key", attack_class_name)

    @staticmethod
    def _split_attack_overrides(kwargs: dict) -> tuple[dict, dict]:
        attack_param_overrides = dict(kwargs.pop("attack_param_overrides", {}) or {})
        general_kwargs = {}

        for key, value in kwargs.items():
            if "__" in key:
                attack_key, param = key.split("__", 1)
                attack_param_overrides.setdefault(attack_key, {})[param] = value
            else:
                general_kwargs[key] = value

        return general_kwargs, attack_param_overrides
    ...
    def run(
        ...
        **kwargs,
        ...
        # E.g. Codec2VocoderAttack with bitrate=[700,1300,2400] becomes
        ...
        for atk_name in attack_types:
            ...
            bitrate_key = next((k for k in config if k == "bitrate" and isinstance(config[k], list)), None)
            if bitrate_key:
        ...
        general_kwargs, attack_param_overrides = self._split_attack_overrides(dict(kwargs))
        ...
        for filepath in filepaths:
            ...
            if crop_before_attack is not None:
                if "CropBeginningAttack" in self.attacks:
                    ...
                    watermarked_audio = pre_crop.apply(
                        watermarked_audio,
                        sampling_rate=sampling_rate,
                        crop_percentage=crop_before_attack,
                    )
            ...
            for attack_class_name, attack_display_name, attack_overrides in expanded_attacks:
                ...
                attack_key = self._attack_key_for_class(attack_class_name)
                current_attack_kwargs.update(
                ...
                if (attack_class_name == "CrossModelAttack"):
                    ...
                    different_model_name = current_attack_kwargs.get(
                        "different_model_name",
                        attack_instance.config.get("different_model_name"),
                    )
                    if not different_model_name:
                        raise ValueError(
                            "CrossModelAttack requires 'different_model_name' in "
                            "config.json or --cross_model.different_model_name."
                        )
                ...
                #in case of the collusion mod attack
                    ...
                    current_attack_kwargs["original_audio"] = audio
