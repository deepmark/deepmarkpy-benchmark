...
plugin's engine class in its own serving layer and images. It never imports this
...
- Each engine's `__init__` loads weights (except `speech_enhancement_2`, which
...
1. one model image: `audio_seal` — serve
   `deepmarkpy.plugins.models.audio_seal.inference.AudioSealEngine` behind
   your own transport, embed and detect a 1 s, 16 kHz test signal;
2. one attack image: `vae` — serve
   `deepmarkpy.plugins.attacks.vae.inference.VAEEngine`, apply it to the
   same signal.
