...
Status: **documented, frozen, deferred** (decided 2026-07-29). Every entry below is a real defect that is
**deliberately not fixed** in the current release line. The
guarantee is bit-for-bit behavior preservation —
...
a dedicated deferred-fix release after v1.0.0.**
...
| D4 | `perth` packs watermark bits then passes `watermark=None` (payload discarded). **Never deletable as "dead code"** | `src/deepmarkpy/plugins/models/perth/inference.py:44-48` | The packing call sequence is preserved verbatim; removing it is a behavior change in disguise |
