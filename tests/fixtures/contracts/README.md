...
| diffusion | attack | stochastic (code evidence) | **recording BLOCKED on the designated machine** — container crashes at startup: the frozen base lineage carries `torchaudio==2.11.0` against `torch==2.7.1`, and torchaudio 2.11's aarch64 wheel dlopens `libcudart.so.13` (import chain: `diffusers` → `transformers` 5.14 `loss_rnnt` → `import torchaudio`). Stochasticity itself is certain from code: `ddpm.py:27-28` draws a fresh OS-entropy seed per request. Escalated: the fix is a dependency version change (owner sign-off required). |
